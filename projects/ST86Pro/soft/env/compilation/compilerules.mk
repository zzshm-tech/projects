...
BINUTILS_VER_INFO := $(word 2,${BINUTILS_VER_CHECK})
...
ifeq "${GCC_VER_RESULT}" "2"
$(call PRINT_INFO,GCC version is unknown - assuming GCC ${GCC_VER_MAJOR}.${GCC_VER_MINOR})
GCC_VER_RESULT := 1
endif # GCC_VER_RESULT == 2

ifeq "${BINUTILS_VER_RESULT}" "2"
$(call PRINT_INFO,Binutils version is unknown - assuming Binutils ${BINUTILS_VER_MAJOR}.${BINUTILS_VER_MINOR})
BINUTILS_VER_RESULT := 1
endif # BINUTILS_VER_RESULT == 2

ifeq "${GCC_VER_RESULT}" "0"

ifneq "${BINUTILS_VER_RESULT}" "0"
$(info Base version: GCC ${GCC_VER_MAJOR}.${GCC_VER_MINOR} / Binutils ${BINUTILS_VER_MAJOR}.${BINUTILS_VER_MINOR})
$(error Binutils version is ${BINUTILS_VER_INFO} whereas GCC version is ${GCC_VER_INFO})
endif
$(call PRINT_INFO,GCC version is ${GCC_VER_INFO})
$(call PRINT_INFO,Binutils version is ${BINUTILS_VER_INFO})
export USE_GCC_4 := 0
export USE_BINUTILS_2_19 := 0
# Garbage collection of unused input sections is not supported
export ENABLE_GC_SECTIONS := 0

else # GCC_VER_RESULT != 0

ifneq "${BINUTILS_VER_RESULT}" "1"
$(info Base version: GCC ${GCC_VER_MAJOR}.${GCC_VER_MINOR} / Binutils ${BINUTILS_VER_MAJOR}.${BINUTILS_VER_MINOR})
$(error Binutils version is ${BINUTILS_VER_INFO} whereas GCC version is ${GCC_VER_INFO})
endif
$(call PRINT_INFO,GCC version is ${GCC_VER_INFO})
...
ifeq "${APART}" "1"
export OBJDUMP_DISPLAY_SECTS := .boot_code .calib .factory_settings .user_data .main_entry_section .sramtext .sramdata .sramucdata .ram .text .ucramdata .data .rotext .rodata .internal_rom .internal_tmp_rom .internal_rom.crc .sramtxt .sysIrqSection .bbIrqSection .sramtextmisc .proxy_memory .boot_rom_version_number
else
...
ifeq "${CYGWIN}" "1"
export LZMA = ${SOFT_WORKDIR}/env/lzma/lzma.exe
else
export LZMA = ${WINE} ${SOFT_WORKDIR}/env/lzma/lzma.exe
endif
...
ifneq "$(CT_PRODUCT)" ""
# Compile target unless generating sector cleaner lods
...
LOCAL_MODULE_DEPENDS := $(sort $(LOCAL_MOD_IS_BCPU_MOD) $(LOCAL_MOD_IS_BCPU_SUBMOD) $(BCPU_MOD_IS_LOCAL_SUBMOD))

GLOBAL_BINARY_LIBS := ${GLOBAL_BCPU_BINARY_LIBS}
LOCAL_BINARY_LIBS := ${LOCAL_BCPU_BINARY_LIBS}
LOCAL_LIB := ${LOCAL_BCPU_LIB}
...
ifeq "${APART}" "1"
LD_SRC := ${SOFT_WORKDIR}/env/link/sl_app_partition.ld
else
...
LDPPFLAGS += \
	...
	-DDUALPORT_SRAM_BASE=${DUALPORT_SRAM_BASE} \
	-DDUALPORT_SRAM_SIZE=${DUALPORT_SRAM_SIZE} \
...
ifeq "${APART}" "1"
LDPPFLAGS += \
	-DSL_FUNCTAB_BASE=${SL_FUNCTAB_BASE} \
	-DSL_FUNCTAB_SIZE=${SL_FUNCTAB_SIZE} \
	-DSL_APP_BASE=${SL_APP_BASE} \
	-DSL_APP_SIZE=${SL_APP_SIZE} 
endif

ifeq "${BCPUGEN}"  "yes"
BCPU_CODE_BASE              ?= 0x1000
BCPU_RAMRUN_CODE_BASE       ?= 0x1500

LDPPFLAGS += \
    -DBCPU_ROM_BASE=${BCPU_ROM_BASE} \
    -DBCPU_ROM_END=${BCPU_ROM_END} \
    -DSYS_SRAM_BCPU_BASE=${SYS_SRAM_BCPU_BASE} \
    -DBCPU_CODE_BASE=${FLASH_BASE}+${BCPU_CODE_BASE}
ifeq "${CT_RAMRUN}" "1"
...
# SPI register debug
ifneq "${filter SPI_REG_DEBUG,${ALL_EXPORT_FLAG}}" ""
...
# BCPU ROM codes are uncached on 8809 (for TCM access), and
# BCPU flash codes would call functions outside 256M address range.
ifeq "${CT_ASIC}" "8809"
ifeq "${BUILD_BCPU_IMAGE}" "1"
ifneq "${BUILD_ROM}" "1"
CFLAGS += -mlong-calls
...
EXTERN_CPPFLAGS = -fno-gnu-keywords -fno-operator-names -fno-exceptions -fno-optional-diags -fno-rtti
...
ifeq "${CT_BIN}" "1"
LODBASE_NO_PATH := ${CT_APP}_${NBR}${CT_TARGET}_${PRODUCT}${CT_RELEASE}_bin
else
...
ifeq "${CT_BIN}" "1"
LODBASE_NO_PATH := $(notdir ${LOCAL_NAME})_${NBR}${CT_TARGET}_${PRODUCT}${CT_RELEASE}_bin
else
...
COMPRESSED_STUB_SRC_DIR := ${SOFT_WORKDIR}/env/compilation/
COMPRESSED_DATA_OBJECTS += ${SOFT_WORKDIR}/lib/get_compressed_data.o
...
ifeq "${TTS}" "1"
BCPU_ELF_FILE_STRIPPED := ${SOFT_WORKDIR}/${LIB_DIR}/TTS/stripped_bcpu_elf_file.elf
else
BCPU_ELF_FILE_STRIPPED := ${SOFT_WORKDIR}/${LIB_DIR}/NO_TTS/stripped_bcpu_elf_file.elf    
endif
endif
...
ifeq "${TTS}" "1"
	cp -rf ${SOFT_WORKDIR}/${LIB_DIR}/TTS/int_sram_overlay.ld ${BUILD_ROOT}/
else
	cp -rf ${SOFT_WORKDIR}/${LIB_DIR}/NO_TTS/int_sram_overlay.ld ${BUILD_ROOT}/
endif
endif
...
ifeq "${TTS}" "1"
	cp -rf ${SOFT_WORKDIR}/${LIB_DIR}/TTS/fast_cpu_memory_overlay.ld  ${BUILD_ROOT}/
else
	cp -rf ${SOFT_WORKDIR}/${LIB_DIR}/NO_TTS/fast_cpu_memory_overlay.ld  ${BUILD_ROOT}/
endif
endif
# all overlays
...
.PHONY: xcp2h xcprealcleanstem xcprealclean xcpfinalcleanstem xcpfinalclean xcpclean
...
ifeq "${CYGWIN}" "1"
WIN_BAS_FINAL := $(shell ${CYGPATH} ${BAS_FINAL})
else
WIN_BAS_FINAL := ${BAS_FINAL}
endif
XMD_FINAL_ABS := ${WIN_BAS_FINAL}.xmd

.PHONY: xmd2h xmdinstall
...
${BIN_PATH}/%.bin.lzma.o: ${BIN_STAGE1} | ${BIN_PATH}
	@${ECHO} "Compress          $*"
	${OBJCOPY} -j .$* -O binary $< ${BIN_PATH}/$*.bin
	cp -f ${BIN_PATH}/$*.bin ${BIN_PATH}/$*.bin.org
	${LZMA} -f -v -9 ${BIN_PATH}/$*.bin
	cd ${BIN_PATH} && ${OBJCOPY} -I binary -O elf32-littlemips -B mips $*.bin.lzma $@
	@${ECHO}
...
STRCMPEX_RUN := ${WINE} ${STRCMPEX_FILE}
endif
...
ifeq "${CT_MMI}" "NGUX"
	...
	@${ECHO} MMI Res Desc Path is ${MMI_CUSTOMER_RES_DESC}
	@${ECHO} MMI Res File Path is ${MMI_CUSTOMER_RES_FILE}
	@${ECHO} Generating Inner Res,Take care of warnings.

	#cd application;${SOFT_WORKDIR}/application/mgapollo/restool/gen_incore_res.sh;cd ..
	${SOFT_WORKDIR}/application/mgapollo/restool/gen_incore_res.sh "${MMI_CUSTOMER_RES_DESC}" "${MMI_CUSTOMER_RES_FILE}" "${CT_ERES}" "application/mgapollo/inner-res" "${SOFT_WORKDIR}/application/mgapollo/restool"
	@${ECHO} Generating Inner Res done.
...
GEN_COMMON_ELF_LD_FILE_FUNC = ${OBJDUMP} -h ${1} | gawk '/ *[0-9]+ +[^ ]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ / \
  { getline attr; if(index(attr,"LOAD")==0) {loadstr="(NOLOAD)"} else {loadstr=""}; \
  vmastr = (strtonum("0x"$$4)==0)?"0x0":"0xffffffff"$$4; \
  lmastr = (strtonum("0x"$$5)==0)?"0x0":"0xffffffff"$$5; \
  printf("%s (%s) %s : AT (%s) { KEEP(${1}(%s)) }\n",$$2,vmastr,loadstr,lmastr,$$2); }'
...
GEN_ROM_ELF_LD_FILE_FUNC = ${OBJDUMP} -h ${1} | gawk --re-interval ' \
  BEGIN { rom_start_str="${GEN_ROM_ELF_LD_ROM_START}"; \
          sub("^0[xX][fF]{8}","0x",rom_start_str); \
          rom_end_str="${GEN_ROM_ELF_LD_ROM_END}"; \
          sub("^0[xX][fF]{8}","0x",rom_end_str); \
          rom_start = strtonum(rom_start_str); \
          rom_end = strtonum(rom_end_str); \
          ...
          printf("%s_%u = .;\n",rom_flash_sym_name,rom_first_section_index); } \
  / *[0-9]+ +[^ ]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ / \
  { \
    getline attr; if(index(attr,"LOAD")==0) {loadstr="(NOLOAD)"} else {loadstr=""}; \
    vmastr = (strtonum("0x"$$4)==0)?"0x0":"0xffffffff"$$4; \
    lma = strtonum("0x"$$5); \
    lmastr = (lma==0)?"0x0":"0xffffffff"$$5; \
    if(lma>=rom_start && lma<=rom_end) { \
        lmastr=sprintf("%s_%u + (0x%08x - 0x%08x)", \
            rom_flash_sym_name,rom_first_section_index,lma,rom_start); } \
    printf("%s (%s) %s : AT (%s) { KEEP(${1}(%s)) }\n",$$2,vmastr,loadstr,lmastr,$$2); \
    if(lma>=rom_start && lma<=rom_end) \
    {\
        printf("%s_%u = %s + SIZEOF(%s);\n", \
            rom_flash_sym_name,rom_section_index+1,lmastr,$$2); \
        rom_section_index++; \
        if(lma>rom_last_section_lma) {rom_last_section_lma=lma;rom_last_section_index=rom_section_index;} \
    } \
  } \
  END {printf(". = %s_%u;\n",rom_flash_sym_name,rom_last_section_index);}'
...
GEN_COMPRESS_STAGE1_ELF_LD_FILE_FUNC = ${OBJDUMP} -h ${1} | gawk '/ *[0-9]+ +[^ ]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ / \
  { getline attr; if(index(attr,"LOAD")==0) {loadstr="(NOLOAD)"} else {loadstr=""}; \
  vmastr = (strtonum("0x"$$4)==0)?"0x0":"0xffffffff"$$4; \
  lmastr = (strtonum("0x"$$5)==0)?"0x0":"0xffffffff"$$5; \
  if ($$2 == ".get_compressed_data") { printf "_get_compressed_data_start = %s;\n",vmastr } \
  else { printf "%s (%s) %s : AT (%s) { KEEP(${BIN_STAGE1_LINKABLE}(%s)) }\n",$$2,vmastr,loadstr,lmastr,$$2 } }'

GEN_COMPRESS_ELF_SECTION_ADDR_FILE_FUNC = ${OBJDUMP} -h ${1} | gawk '/ *[0-9]+ +[^ ]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ +[0-9a-zA-Z]+ / \
  { if (strtonum("0x"$$4)==0) next; \
  sizestr = "0x"$$3; \
  vmastr ="0x"$$4; \
  lmastr ="0x"$$5; \
  if ($$2 == ".get_compressed_data") { printf "%s <> %s %s\n",$$2,vmastr,lmastr } \
  else if ($$2 != ".user_data") { printf "%s %s %s %s\n",$$2,sizestr,vmastr,lmastr } }'
...
# With compression
ifeq "${COMPRESS_PROCESS_ENABLE}" "yes"
BIN_VAR_OPTIONAL_PREREQUISITES += ${COMPRESSED_DATA_OBJECTS}
...
ifneq "$(filter 1,${ROM_IN_FLASH_ELF} ${REWRITE_ROM_AT_BOOT})" ""
...
ifneq "${COMPRESS_PROCESS_ENABLE}-${LINK_STAGE_2}" "yes-yes"
	...
	${OBJCOPY} -R .get_compressed_data ${BIN_STAGE1} ${BIN_STAGE1_LINKABLE}
...
SKIP_BIN_GEN_FOR_BCPU := 0
ifeq "${BUILD_BCPU_IMAGE}" "1"
ifeq "$(strip ${LOCAL_MODULE_DEPENDS} ${FULL_SRC_OBJECTS})" ""
SKIP_BIN_GEN_FOR_BCPU := 1
endif
endif

ifeq "${SKIP_BIN_GEN_FOR_BCPU}" "1"
# Also remove bin files generated in previous build if any
bin: locclean
ifneq "${APART}" "1"
	@${ECHO}
	@${ECHO} "BCPU		  No ELF to generate"
endif
else # !SKIP_BIN_GEN_FOR_BCPU
...
ifeq "${APART}" "1"
				${OBJCOPY} -R .functab -O binary ${BIN} ${BAS_FINAL}.bin 
				perl ${APP_LOD_GEN_SCRIPT_PATH}/${APP_LOD_GEN_SCRIPT} ${BAS_FINAL}.bin ${SL_APP_LOD_BASE} ${SL_APP_SIZE} ${FLSH_MODEL} ${SL_APP_LOADINFO_BASE} ${SL_APP_LOADINFO_SIZE}
else
...
ifeq "${APART}" "1"
					@${ECHO} "${LODBASE_NO_PATH}.bin/lod Generated"
else
...
lod:
ifeq "${APART}" "1"
	cp ${SOFT_WORKDIR}/lib/sl_app.c ${SOFT_WORKDIR}/app/${CT_PRODUCT}/src/
else
	rm -f ${SOFT_WORKDIR}/app/${CT_PRODUCT}/src/sl_app.c
endif
...
build_tags: clean_tags
	ctags -f ${TAGS_FILE} -h +hp -R ${SOFT_WORKDIR}/app
