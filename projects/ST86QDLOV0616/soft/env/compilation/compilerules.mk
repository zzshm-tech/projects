...
export LIBGCC_PATH := 
# Try to get the GCC tool-chain lib path
GCC_PATH := $(shell which $(CC))
ifneq "${GCC_PATH}" ""
LIBGCC_PATH := $(dir ${GCC_PATH})../lib/gcc/$(patsubst %-,%,${CROSS})/${GCC_VER_INFO}
ifeq "$(wildcard ${LIBGCC_PATH})" ""
LIBGCC_PATH := 
endif
endif
# Use the GCC libs in repository in case of failure
ifeq "${LIBGCC_PATH}" ""
ifeq "${USE_GCC_4}" "1"
LIBGCC_PATH := ${SOFT_WORKDIR}/env/lib/gcc/mips-elf/4.4.2
else
LIBGCC_PATH := ${SOFT_WORKDIR}/env/lib/gcc/mips-elf/3.4.6
endif
endif
...
ifeq "${REDUNDANT_DATA_REGION}" "1"
LDPPFLAGS += -DREDUNDANT_DATA_REGION
else
LDPPFLAGS += -REDUNDANT_DATA_REGION
endif
...
LDPPFLAGS += -DBCPU_CODE_BASE=${FLASH_BASE}+${BCPU_CODE_BASE}
endif
...
# sky 20161025 modify for add time info after firmware version.
SL_BUILD_TIME := $(shell date '+%Y%m%d')

ifneq "$(CT_PRODUCT)" ""
PRODUCT := ${CT_PRODUCT}_${SL_BUILD_TIME}_
else
PRODUCT :=
endif # CT_PRODUCT
...
BIN_FINAL := ${BAS_FINAL}_${SL_VER}.elf
MAP_FINAL := ${BAS_FINAL}_${SL_VER}.map
HEX_FINAL := ${BAS_FINAL}_${SL_VER}.srec
DIS_FINAL := ${BAS_FINAL}.dis
XMD_FINAL := ${BAS_FINAL}.xmd
XCP_FINAL := ${BAS_FINAL}.xcp
DLL_FINAL := ${BAS_FINAL}.dll
LODBASE   := ${BAS_FINAL}_${SL_VER}_
...
ifeq "${WITH_GIT}" "1"
LOCAL_REVISION_CMD := { git log -1 --oneline ${LOCAL_PATH} || echo "FFFFFFFF"; } | cut -d' ' -f1 | sed 's/^/0x/'
    ...
    while(<>) { \
        if (/^ *[0-9]+ +([^ ]+) +[0-9a-zA-Z]+ +([0-9a-zA-Z]+) +([0-9a-zA-Z]+) /) { \
            ...
            if ($$secstr eq ".get_compressed_data") { \
                ...
                printf("$$secstr ($$vmastr) $$loadstr : AT ($$lmastr) { KEEP(${BIN_STAGE1_LINKABLE}($$secstr)) }\n"); \
            } \
        } \
    }'
