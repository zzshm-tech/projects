...
**	���˵��:
**	SL_NW_REG_STATUS״ֵ̬��
**	SL_NW_STATUS_NOTREGISTERED_NOTSEARCHING =  0,// Not registered and not searching. //ûע��
**  SL_NW_STATUS_REGISTERED_HOME =             1,//Registered and in the home area.   //�ڱ���ע�����
**  SL_NW_STATUS_NOTREGISTERED_SEARCHING =     2,//Not registered and searching for a new operator. 
**  SL_NW_STATUS_REGISTRATION_DENIED =         3,//Registration denied.    //ע�ᱻ�ܾ�
	...
	*/
	...
	switch(step)
	{
		...
		case 2:               								//��ȡSim��״̬  
			...
			SL_Print("Register Net............%d\r\n",TcpIpSocket.NetWorkState);   //ע������
			break;
		case 3:              //���ý��뷽ʽ
			slRet = SL_TcpipGprsNetGetState(&TcpIpSocket.GprsNetActive,&TcpIpSocket.NetGprsState);  //��ȡ
			if(slRet == SL_RET_OK)
			{
				SL_Print(">App:NetGprsState:%d,GprsNetActive %d\r\n",TcpIpSocket.NetGprsState,TcpIpSocket.GprsNetActive);
				if(TcpIpSocket.GprsNetActive != 1)    //�ڴ˴�ֻ�ж�ʱ���Ѿ����������� 
				{
					pCfg = GetSysCfgSpace();
		...
		case 5:
			...
			break;
		case 6:
			if(TcpIpSocket.LinkType == 1)            					//FTP����
			{
				step = 15;                                					//
				break;
			}
			slRet = SL_TcpipSocketCreate(0, SL_TCPIP_SOCK_TCP);   	 //��������   ����Socket��Ϊ0��
			if(slRet != SL_RET_OK)
    		{
				step = 2;                            		//��������
				break;
		...
		case 7:             //�ò����ȡ���ص�ַ��������Ƿ�������������������������Ӧ������ȡ����IP��ַ��
			...
			if(tmp == 0)   //˵��������
			{
				...
				TcpIpSocket.UlrToIPFlag = 0;  //���ﲻ�ٽ���ִ���Ƿ�ɹ����ж�
				step++;
				erro_code = 0;
				counter = 0;
				break;
		...
		case 10:
			if(TcpIpSocket.LinkState == 0 && counter++ < 50)   		  //�ȴ� 50 s
				break;
			if(TcpIpSocket.LinkState == 3 || TcpIpSocket.LinkState == 0)  //����ʧ��
			{
				step = 2;
		...
		case 11:
			...
			if(slRet == SL_TCPIP_STATE_CONNECTOK)               //�����������
			{
				...
				TcpIpSocket.GprsLinkState = 2;				  //���ӷ������ɹ�  
		...
		case 18:
			...
			break;
		case 19:                             //�������� 
			if(TcpIpSocket.FtpLoadFig == 1 && counter++ < 120)             // 2����
				break;
		...
		case 22:
			...
			break;
		case 23:
			slRet = SL_TcpipGprsNetDeactive();          //�ر�������
			if(slRet != SL_RET_OK)                     	//���ִ��ʧ��
			{
				if(erro_code++ > 5)                     //�����豸
					step = 14;
...
void ProcessGprsRecv(void)
{
	...
	if(pSysCmd->FrameStart[0] != 0xFF && pSysCmd->FrameStart[1] != 0xFF && pSysCmd->FrameStart[2] != 0xFF)   //�жϰ�ͷ
		return;
	
	tmp = VerfySum(GprsRecvBuf,GprsRecvLen - 3);    			//����У����  
	         						 							
	CmdID = 32 * PowTow((double)tmp) + 23 * tmp + 71;
	TmpData.TTbyte[0] = *(GprsRecvBuf + GprsRecvLen - 3);   	//
	TmpData.TTbyte[1] = *(GprsRecvBuf + GprsRecvLen - 2);   	//
	
	if(CmdID != TmpData.IntII)          	   						//�ж�У��
		return;
	
	if(pSysCmd->TypeCode != 0x06)           //�ж�ID�ж���ϢID��
		return;
...
{	
	...
	switch(ComCmdRes.CmdID)
	{
		...
		case 2:					//
				...
				case 0x15:     //�ظ�
					...
					if(Counter < 35 && pLockCar->MonCurrentState == 0)
					{
						...
						*(Buf + tmp) = 0x01;				//����״̬
						...
						ComCmdRes.MsgLen = tmp - 6;
						ComCmdRes.DataLen = tmp - 9;
						SL_Memcpy(Buf,(unsigned char *)&ComCmdRes,sizeof(ComCmdRes));  //�����豸ͷ 
						TmpData16.TTbyte[0] = VerfySum(Buf,tmp);								  //У��  
						TmpData16.IntII = (unsigned short int)(32 * PowTow((double)TmpData16.TTbyte[0]) + 23 * TmpData16.TTbyte[0] + 71);  //����У��ֵ
						Buf[tmp++] = TmpData16.TTbyte[0];
						Buf[tmp++] = TmpData16.TTbyte[1];
						Buf[tmp++] = 0x0D;
						...
						WriteCmdDataBuf(Buf,tmp);
						ResetHeatSw();                             //��λ������
						SL_Memset((unsigned char *)&ComCmdRes,0,sizeof(ComCmdRes));   //����������ݿռ�
			...
			break;
		case 0x06:		//�ظ�Զ������������յ������ʱ�ظ��������Ƿ�ɹ���Ҫ�������Ͱ汾���ж�
		...
		case 0x03:								//�ظ���������
			...
			if(Counter < 35 && pLockCar->LockCurrentState == 0)
			{
				if(pLockCar->LockCarCmd == 1)   //�������� 
				{
					*(Buf + tmp) = 0x01;	//����״̬  --��������
					tmp++;
					*(Buf + tmp) = pLockCar->LockCarRank;   			//һ�������ɹ���
					tmp++;
				}
			...
			else                     //����������ʧ��
			{
				if(pLockCar->LockCarCmd == 1)   //�������� 
				{
					*(Buf + tmp) = 0x01;	//����״̬  --��������
					tmp++;
					*(Buf + tmp) = pLockCar->LockCarRank + 0x10;   			//����δִ��
					tmp++;
				}
				else if(pLockCar->LockCarCmd == 2)  //��������
				{
...
void AppSmsRcvCb(S32 slResult, U8 ucIndex, U8* pucbuf, U16 usLenth, U8* pucTelNumber)
{
	...
	if(SL_SmsGetSmsFormat())
	{
		...
		SL_Memset(SMSRecvBuf, 0, 100);		 					//����Ž��ջ���
