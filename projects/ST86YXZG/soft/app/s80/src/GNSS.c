...
static unsigned char 			GnssDataBuf[1000];    		//
...
unsigned char GnssDataParse(void)
{
	...
	m_data_len =  ReadComPkt(2,GnssDataBuf,sizeof(GnssDataBuf));   //
	...
	switch(step1)            //
	{
		...
		case 1:
			...
			//SL_Print("Run This.......\r\n");
			if(read_stopwatch16_value(&sw) < 4800)    //  
				break;
			...
			break;
		case 2:
			if(read_stopwatch16_value(&sw) < 200)    //  
				return 0;
	...
	if(counter > 100)
	{
		...
		//GnssLedOff();
		counter--;
		GnssData.longitude_ew = 1;
	}
	
	if(m_data_len == 0)
		return 0;
	GnssData.longitude_ew = 0;
	counter = 0;
	...
	if((index > 0) && (p + 100) < endof(GnssDataBuf))     //
	{
		...
		if(m_str_len > 0)                                 								//
		{
				if(GetDataStr(2,3,p,TempBuf,m_str_len) > 0)    							//
				{
					...
					if(TempBuf[0] != 'A')                                       	//
					{
						...
						GnssStatusBack = 'V';
						//GnssLedOff();
						//SL_Print("Gnss Mode Is %c ,%u\r\n",GnssData.status,GnssData.gnss_time);
						//return 1;            //如果设备未定位
					}
					else
					{
		...
		TempGnssData.gnss_time = CalendarToUnix(tmp_time) + 28800;          //28800  (八个小时)
		TempGnssData.speed *= 1.85;                                         //   
		//SL_Print("\r\n")
		GnssData.gnss_time = TempGnssData.gnss_time;
		...
		//SL_Print("The Gnss Latitude:%u The Gnss Longitude:%u,%u\r\n",tmp1,tmp2,TempGnssData.status );
