...
static unsigned char 				MonstateTmp;
/**********************************************/
...
static unsigned char 			CurrentLockCarState = 0; //实际锁车状态备份
...
static void SendReplaceDevCmd(void)
{
	...
	TerSendBuf[11] = 0x55;
	TerSendBuf[12] = 0xAA;
	SL_UartSendData(SL_UART_1,TerSendBuf,13);
}
...
unsigned char ProTermRecvData(void)
{
	...
	if(DataLen == 0)
		return 0;
	init_stopwatch16(&sw);
	WriteDeviceAlram(2,0);           //清除MCU断开标志
	...
	switch(tmp)
	{
		...
		case 0x0001:                                     			//
			...
			LockCar.MonCurrentState = TerStateData.MonState;    	//开启监控命令状态
			
			(TerStateData.PowerVol < 50) ? WriteDeviceAlram(0,1) : WriteDeviceAlram(0,0);  //外部电源电压断开报警
			
			(TerStateData.PowerVol >= 50 && TerStateData.PowerVol < 60) ? WriteDeviceAlram(1,1) : WriteDeviceAlram(1,0);   //外部电源电压低报警  报警
				
			(TerStateData.ShellState > 0) ? WriteDeviceAlram(3,1) : WriteDeviceAlram(3,0);   //开外壳报警

			(TerStateData.CanConnect > 0) ? WriteDeviceAlram(4,1) : WriteDeviceAlram(4,0); 	//CAN链接断开

			(TerStateData.GnssAntState > 0) ? WriteDeviceAlram(5,1) : WriteDeviceAlram(5,0);  //GNSS天线断开报警

			(TerStateData.GnssAntShort > 0) ? WriteDeviceAlram(6,1) : WriteDeviceAlram(6,0);  //GNSS天线短路报警 
			...
			if(CanData.TempWater < 40)
				CanData.TempWater = 40;

			if(CanData.OilTemp < 40)
				CanData.OilTemp = 40;
			
			if(CanData.EngineTorque < 125)
				CanData.EngineTorque = 125;
			
			if(CanData.AirTemp < 273)
				CanData.AirTemp = 273;

			if(CanData.EnteredAirTemp < 40)
				CanData.EnteredAirTemp = 40;
			
			if(CanData.FuelTemp < 40)
				CanData.FuelTemp = 40;
			...
			{
				TerStateData.AccState = 0;
			}

			if(CanData.SysVol < 10)
				TerStateData.AccState = 0;
			//SL_Print("CanData Data:%d,%d,%d\r\n",TerStateData.CanConnect,CanData.SysVol,TerStateData.AccState); 
			if(TerStateData.FirNum == 15 || TerStateData.FirNum == 14)            //添加这一段，是为了解决单片机不能正常升级的问题，
			{
				//SL_Print("The Mcu Ver:%d\r\n",TerStateData.FirNum == 15);
				TerStateData.cmd_hand = 0;
				TerStateData.cmd_lock = 0;
				TerStateData.cmd_mon = 0;
				TerStateData.cmd_unlock = 0;
				TerStateData.cmd_unmon = 0;
				
				if(TerStateData.AccState == 0)         //20200317修改内容
				{
					CanData.LockCarState = pSysRun->LockCarState;
					CanData.KeyStatus = pSysRun->KeyStatus;
					CanData.TerIDStatus = pSysRun->TerIDStatus;
					CanData.ActivateStatus = pSysRun->ActivateStatus;
					CanData.EngineWorkTime = pSysRun->CanECUWorkTime;
					CanData.OilPosition = pSysRun->CanOilPosition;
				}
				else
				{
					pSysRun->LockCarState = CanData.LockCarState;
					pSysRun->KeyStatus = CanData.KeyStatus;
					pSysRun->TerIDStatus = CanData.TerIDStatus;
					pSysRun->ActivateStatus = CanData.ActivateStatus;
					pSysRun->CanECUWorkTime = CanData.EngineWorkTime;
					pSysRun->CanOilPosition = CanData.OilPosition;
				}
				

				if(pSysRun->LockCarBank > 0)
				{
					if(CanData.LockCarState == 0)
					{
						CanData.LockCarState = 9;
						CurrentLockCarState = 0;
					}
					if(CanData.LockCarState == 1) 
					{
						CanData.LockCarState = (pSysRun->LockCarBank > 2) ? 2 : pSysRun->LockCarBank;
						CurrentLockCarState = CanData.LockCarState;
					}
				}
				else
				{
					if(CanData.LockCarState == 0)
					{
						CanData.LockCarState = 0;
						CurrentLockCarState = 0;
					}
					if(CanData.LockCarState == 1) 
					{
						CanData.LockCarState = CurrentLockCarState;
					}
				}
			...
			//SL_Print("The Local Log:%d,%d,%d,%d,%d\r\n",TerStateData.cmd_hand,TerStateData.cmd_lock,TerStateData.cmd_unlock,TerStateData.cmd_mon,TerStateData.cmd_unmon);
			...
			TerTime.VerifyBit = 1;         			 //
			WriteSleepStatus(*p);   				 //
			//SL_Print("The Mcu Status 1::%d\r\n",*p);
			//SL_Print("CanData.EngineWorkTime :%d,%d,%d\r\n",CanData.ActivateStatus,CanData.KeyStatus,CanData.TerIDStatus);  
			//SL_Print("Can Lock Status:%d\r\n",CanData.LockCarState);
			p++; 
			...
			//SL_Print("The Mcu Status::%d,%d,%d,%d\r\n",CanData.EngineNacelleTemp,CanData.AirTemp,CanData.EnteredAirTemp,CanData.EnteredAirTemp);
			//SL_Print("The Mcu Status::%d\r\n",CanData.CarWarnValue2);
			//SL_Print(">The Current FaultCode Num:%d\r\n",CurrentFaultCode.FaultCodeNum);
			break;
		case 0x0022:                                          //
		...
		case 0x00FE:                //
			...
			if((i = LookForStr(p,"BDWMODIF",DataLen - 6)) >= 0)   		//
			{
				...
				if(tmp > 0)
				{
					...
					if(pSysCfg->CanProtocolNum == 0x30 )                       //
						pSysCfg->CarType = 0x02; 
					else if(pSysCfg->CanProtocolNum == 0x31)                   	//
						pSysCfg->CarType = 0x02;   
					else
					{
						pSysCfg->CanProtocolNum = 0x30;                       	//
